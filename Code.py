...
correlation matrix, county-level bar charts, distribution
histograms, and a statewide choropleth map — all saved to outputs/.
...
# ── Fig 5 — Statewide choropleth map (diabetes prevalence) ───
# Extract lat/lon from geolocation field
...
scatter = ax.scatter(
    ...
    cmap="YlOrRd", s=6, alpha=0.7, edgecolors="none",
...
cbar.set_label("Diabetes Prevalence (%)", fontsize=11)
ax.set_title("Diabetes Prevalence by Census Tract\nWashington State — CDC PLACES 2024",
...
ax.set_aspect(1.3)
plt.tight_layout()
fig.savefig(os.path.join(OUT, "fig5_wa_diabetes_map.png"), dpi=180)
plt.close(fig)
print("  ✓ fig5_wa_diabetes_map.png")
...
summary = sub[list(VARS.keys())].describe().loc[["mean", "std", "min", "max"]].T
